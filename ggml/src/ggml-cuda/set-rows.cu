...
// Turbo3 group kernel: each thread processes 128 elements -> 4 turbo3 blocks
// Required because turbo3 WHT rotation spans 128 elements but block size is 32
...
static __global__ void k_set_rows_turbo3(const float * __restrict__ src0,
    ...
    const float * src_ptr = src0 + i01*s01 + i02*s02 + i03*s03 + i00;
    block_turbo3_0 * dst_ptr = dst + (dst_row*s1 + i02*s2 + i03*s3) / sizeof(block_turbo3_0) + i00 / QK_TURBO3;

    quantize_f32_turbo3_0_group(src_ptr, dst_ptr);
...
static void set_rows_cuda_turbo3(
    ...
    // Total number of 128-element groups across all dimensions
    const int64_t ne_total = (ne00 * ne01 * ne02 * ne03) / QK_TURBO3_GROUP;
    const int num_blocks = (ne_total + CUDA_SET_ROWS_BLOCK_SIZE - 1) / CUDA_SET_ROWS_BLOCK_SIZE;
    const dim3 block_size(CUDA_SET_ROWS_BLOCK_SIZE);
    const dim3 grid_size(num_blocks);
